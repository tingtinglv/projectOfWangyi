...
<html xmlns="http://www.w3.org/1999/xhtml" lang="en">
	...
	<body>
		<header class="main">
			...
			<form name="department_changed" action="/neusoft/DepChangedServlet.action">
		...
		<%
              ...
              {
               %>
			   <li><a href="DepChangedServlet.action?currentPage=${page.currentPage -1}&syear=<%=syear %>&smonth=<%=smonth %>&sday=<%=sday %>&eyear=<%=eyear %>&emonth=<%=emonth %>&eday=<%=eday %>&empseq=<%=empseq %>&empname=<%=empname %>&type=<%=type %> ">上一页</a></li>
               <%
               }
               if(mypage.getTotalPage()<=7)
               {
               		...
               		{
               	%>
               <li><a href="DepChangedServlet.action?currentPage=<%=i%>&syear=<%=syear %>&smonth=<%=smonth %>&sday=<%=sday %>&eyear=<%=eyear %>&emonth=<%=emonth %>&eday=<%=eday %>&empseq=<%=empseq %>&empname=<%=empname %>&type=<%=type %>" id=<%=i %>><%=i %></a></li>
               	<%
               		}
               }
               else
               {
               		...
               		%>
               		<li><a href="#">...</a></li>
               		 <li><a href="DepChangedServlet.action?currentPage=${page.totalPage}&syear=<%=syear %>&smonth=<%=smonth %>&sday=<%=sday %>&eyear=<%=eyear %>&emonth=<%=emonth %>&eday=<%=eday %>&empseq=<%=empseq %>&empname=<%=empname %>&type=<%=type %>">末页</a></li>
               		<%
               		}
               		else if((mypage.getTotalPage()-mypage.getCurrentPage())<=3)
               		{
               		%>
               		 <li><a href="DepChangedServlet.action?currentPage=1&syear=<%=syear %>&smonth=<%=smonth %>&sday=<%=sday %>&eyear=<%=eyear %>&emonth=<%=emonth %>&eday=<%=eday %>&empseq=<%=empseq %>&empname=<%=empname %>&type=<%=type %>">首页</a></li>
               		<li><a href="#">...</a></li>
               		<%
              ...
              {
               %>
                <li><a href="DepChangedServlet.action?currentPage=${page.currentPage +1}&syear=<%=syear %>&smonth=<%=smonth %>&sday=<%=sday %>&eyear=<%=eyear %>&emonth=<%=emonth %>&eday=<%=eday %>&empseq=<%=empseq %>&empname=<%=empname %>&type=<%=type %> ">下一页</a></li>
               <%
               }
                %>
		</ul>
