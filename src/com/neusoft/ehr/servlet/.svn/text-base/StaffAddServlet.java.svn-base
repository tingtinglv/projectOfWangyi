...
public class StaffAddServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		ProbationService fs = new ProbationService();
		...
		boolean judge = false;
		if(probation.equals("是")){
			type="试用期";
			judge = ss.add(s);
			 Date startDate = Date.valueOf(request.getParameter("startDate"));
			 Date endDate = Date.valueOf(request.getParameter("endDate"));
			int empId =ss.findBySeq(empSeq).getEmpId();
			java.util.Date date = new java.util.Date();
			Date checkTime = new Date(date.getTime());
			p.setEmpId(empId);
		...
		request.getRequestDispatcher("/StaffEntryInfoServlet.action").forward(request, response);
	}
